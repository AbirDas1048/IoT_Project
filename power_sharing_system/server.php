...
// connect to database
	...
	if (isset($_POST['sell'])) {
		...
		if (empty($voltage)) { array_push($errors, "Power is required"); }
		...
		$output = floatval($output);
		$minimum_share = 9900 - $output;
		if ($voltage > $minimum_share) {
			array_push($errors, "You can't share power more than $minimum_share unit");
		}
